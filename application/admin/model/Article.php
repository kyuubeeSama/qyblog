...
class Article extends model {
		...
		if ( $this->create( $data ) ) {
			...
			if ( $aid = $this->insert() ) {
				...
				if ( ! empty( $image_path ) ) {
					...
					// 传递图片插入数据库
					\model( 'ArticlePic' )->addData( $aid, $image_path );
				}
				// 获取未删除和展示的文章
				$sitemap_map = array(
					'is_show'   => 1,
	...
	public function getPageData( $cid = 'all', $tid = 'all', $is_show = '1', $is_delete = 0, $limit = 10 ) {
		...
		foreach ( $list as $k => $v ) {
			...
			$list[ $k ]['category'] = current( \model( 'Category' )->getDataByCid( $v['cid'], 'cid,cid,cname' ) );
	...
	public function getDataByAid( $aid, $map = '' ) {
		if ( $map == '' ) {
			...
			$data['current']['category'] = current( \model( 'Category' )->getDataByCid( $data['current']['cid'], 'cid,cid,cname,keywords' ) );
	...
	public function getDataByTitle( $search_word ) {
		...
		foreach ( $list as $k => $v ) {
			...
			$list[ $k ]['tids']     = \model( 'ArticleTag' )->getDataByAid( $v['aid'] );
			$list[ $k ]['tag']      = \model( 'ArticleTag' )->getDataByAid( $v['aid'], 'all' );
			$list[ $k ]['category'] = current( \model( 'Category' )->getDataByCid( $v['cid'], 'cid,cid,cname,keywords' ) );
			$list[ $k ]['addtime']  = word_time( $v['addtime'] );
		}
		$show = $page->show();
		$data = array(
